...
def creatingDirectories(path):
     # Creating directories to store images
        ...
        if not os.path.exists(path+'\SobelEdges'):
            os.mkdir(path+'\SobelEdges')  
        if not os.path.exists(path+'\CannyEdges'):
            os.mkdir(path+'\CannyEdges')
        if not os.path.exists(path+'\HoughLines'):
            os.mkdir(path+'\HoughLines')
