...
def computeSvm(train,tags,kernelType='linear'):
    #clf = OneVsRestClassifier(svm.SVC(kernel=kernelType,probability=True))
    #clf = svm.LinearSVC()
    clf = svm.SVC(kernel=kernelType,probability=True)
    clf.fit(train,tags)
    return clf
...
def predictProba(clf,test):
    score_prob = clf.predict_proba(test)    
    return score_prob
...
def vectorsOfTraingTestTags(vector,tag,names):
    ...
    for i in range(0,len(vector)):
        ...
        if (i <= trainSize):
            ...
            # create the test vector and the names one
